...
`cc` 命令用于编译C语言源代码文件，将其转换为可执行文件。这个过程通常包括**预处理、编译、汇编和链接**几个步骤。

---
## 语法
### 基本用法

- **编译单个源文件**：这条命令会将 `source_file.c` 编译成名为 `output_file` 的可执行文件。
  ```shell
  cc -o output_file source_file.c
  ```
- **包含头文件搜索路径**：使用 `-I` 选项可以指定头文件的搜索路径。
  ```sh
  cc -I/include/path source_file.c
  ```
- **链接库文件**：`-L` 指定库文件搜索路径，而 `-l` 用于链接名为 `libname.a` 或 `libname.so` 的库文件。
  ```sh
  cc -o output_file source_file.c -L/library/path -lname
  ```
- **编译并链接多个源文件**：可以同时编译和链接多个源文件。
  ```sh
  cc -o output_file file1.c file2.c file3.c
  ```
- **调试信息**：使用 `-g` 选项可以在编译的程序中包含调试信息，这样就可以使用调试器进行调试。
  ```sh
  cc -g source_file.c
  ```
- **优化代码**：`-O` 选项用于优化代码，`-O2` 表示二级优化。
  ```sh
  cc -O2 source_file.c
  ```
- **显示警告信息**：`-Wall` 选项会开启编译器的所有警告。
  ```sh
...
- 查阅更详细的文档或手册页（执行 `man cc`）来获取更全面的信息。