...
3.1.2.  Mgmt
   ...
   例如，定义初始互联网标准MIB的RFC将被分配管理文档编号1。该RFC在定义互联网标准MIB时，将使用对象标识符：{ mgmt 1 }即：1.3.6.1.2.1 生成互联网标准MIB的新版本是一个严格的过程。本备忘录第5节描述了定义新版本时所遵循的规则。
...
3.1.3.  Experimental
   ...
   the Internet.
   experimental(3)子树用于标识互联网实验中使用的对象。experimental(3)子树的管理工作由IAB委托给互联网号码分配管理局（IANA）负责。
...
3.1.4.  Private
   ...
   在接收到一个子树后，企业可以（例如）在该子树下定义新的 MIB 对象。此外，**强烈建议**企业也在该子树下注册其网络子系统，以便为管理协议提供一种明确的识别机制。例如，如果“弗林特斯通股份有限公司”企业生产了网络子系统，那么他们可以向互联网号码分配机构（IANA）申请一个位于 `enterprises` 子树下的节点。

      1.3.6.1.4.1.42

   The "Flintstones, Inc." enterprise might then register their "Fred
   Router" under the name of:
   “弗林特斯通股份有限公司”（Flintstones, Inc.）企业随后可能会以如下名称注册其“弗雷德路由器”（Fred Router）
...
3.2.  Syntax
   ...
   语法（Syntax）用于定义与对象类型相对应的结构。虽然ASN.1的完整通用特性不被允许使用，但仍会使用ASN.1的构造来定义这种结构。

   The ASN.1 type ObjectSyntax defines the different syntaxes which may
   be used in defining an object type.
   ASN.1 类型 `ObjectSyntax` 定义了可用于定义对象类型的不同语法。
...
3.2.1.  Primitive Types
   ...
   只允许使用 ASN.1 的原始类型 `INTEGER`、`OCTET STRING`、`OBJECT IDENTIFIER` 和 `NULL`。这些类型有时也被称为**非聚合类型（non-aggregate types）**。
...
RFC 1155                          SMI                           May 1990
   ...
   如果枚举整数被列为对象类型，则枚举列表中不得包含值为0的命名编号。禁止使用该值。
...
3.2.2.  Constructor Types
   ...
   允许使用ASN.1构造函数类型SEQUENCE，前提是它用于生成列表或表。对于列表，语法形式如下：
   ...
   其中每个<type>均解析为上文所列的ASN.1基本类型之一。此外，这些ASN.1类型始终存在（DEFAULT和OPTIONAL子句不出现于SEQUENCE定义中）。
...
3.2.3.  Defined Types
   ...
   此外，还可以定义新的应用全局类型，只要它们能解析为隐式定义的ASN.1基本类型、列表、表格或其他某种应用全局类型。最初定义的应用全局类型很少。一旦达成共识，未来的备忘录无疑会定义其他类型。
...
3.2.3.1.  NetworkAddress
   ...
   这个CHOICE表示来自可能多个协议族之一的地址。目前，该CHOICE中仅存在一个协议族，即互联网协议族。
...
3.2.3.2.  IpAddress
   ...
   这个应用全局类型表示一个32位的互联网地址。它表示为长度为4的八位位组字符串，采用网络字节序。

当使用ASN.1基本编码规则对该ASN.1类型进行编码时，只能使用基本编码形式。
...
RFC 1155                          SMI                           May 1990
   ...
   counters.
   这个应用全局类型表示一个非负整数，它单调递增直至达到最大值，然后回绕并从零开始再次递增。本备忘录规定计数器的最大值为 2^32-1（即十进制 4294967295）。
...
3.2.3.4.  Gauge
   ...
   gauges.
   这个应用全局类型表示一个非负整数，它可能增加或减少，但会在达到最大值时保持不动（即达到最大值后不再增加）。本备忘录规定仪表（Gauge）的最大值为 2^32-1（即十进制 4294967295）。
...
3.2.3.5.  TimeTicks
   ...
   这个应用全局类型表示一个非负整数，用于统计自某个纪元以来以百分之一秒为单位的时间。当在MIB中定义使用此ASN.1类型的对象类型时，该对象类型的描述会指明参考纪元。
...
3.2.3.6.  Opaque
   ...
   这个应用全局类型支持传递任意ASN.1语法的能力。一个值使用ASN.1基本编码规则编码为一个八位位组字符串。然后，该字符串又作为OCTET STRING进行编码，实际上是对原始ASN.1值进行了“双重包装”。

请注意，一致性实现只需能够接收和识别不透明编码的数据即可。它不需要能够解包数据并解释其内容。

还需注意，通过使用ASN.1 EXTERNAL类型，不透明编码的数据中可以使用ASN.1以外的编码方式。
...
3.3.  Encodings
   ...
   一旦对象类型的实例被识别，其值即可通过对该对象类型的语法应用ASN.1基本编码规则来传输。
...
4.  Managed Objects
   ...
   尽管本备忘录的目的并非定义MIB中的对象，但本备忘录规定了供其他定义这些对象的备忘录所使用的格式。
   ...
   Definition:
      ...
      对象类型语义的文本描述。由于本MIB旨在用于多厂商环境中，实现应确保其对象实例符合此定义。因此，确保对象在所有机器上具有一致的含义至关重要。

   Access:
      One of read-only, read-write, write-only, or not-accessible.
      只读（read-only）、读写（read-write）、只写（write-only）或不可访问（not-accessible）之一。

   Status:
      One of mandatory, optional, or obsolete.
      其中之一：强制（mandatory）、可选（optional）或废弃（obsolete）。

   Future memos may also specify other fields for the objects which they
   define.
   未来的备忘录也可能为其所定义的对象指定其他字段。
...
4.1.  Guidelines for Object Names
   ...
   future extensions.
   互联网标准MIB中的任何对象类型，其名称中均不得使用值为0的子标识符。该值保留供未来扩展使用。
   ...
   user interfaces.
   互联网标准MIB中每个对应对象类型的对象描述符（OBJECT DESCRIPTOR）都应是唯一且便于记忆的可打印字符串。这有助于人们在讨论MIB时使用共同的语言，也便于用户界面进行简单的表格映射。
