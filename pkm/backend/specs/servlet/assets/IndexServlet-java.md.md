                ...
                <head>
                    ...
                    <style>
                        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Arial', sans-serif; }
                        ...
                        .header h1 { color: #333; }
                        .add-btn { padding: 10px 20px; background-color: #4a90e2; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 14px; }
                ...
                <body>
                    ...
                    <script>
                            ...
                            fetch('/web08/api/list')
                                    ...
                                    data.forEach(function(d) {
                                        tbody.innerHTML +=
                                            '<tr>' +
                                                ...
                                                '<td>' + d.loc + '</td>' +
                                                '<td>' +
                                                    '<button class="action-btn view-btn" onclick="viewDept(' + d.deptno + ')">View</button>' +
                                                    '<button class="action-btn edit-btn" onclick="editDept(' + d.deptno + ')">Edit</button>' +
                                                    '<button class="action-btn delete-btn" onclick="deleteDept(' + d.deptno + ')">Delete</button>' +
                                                '</td>' +
                                            '</tr>';
                                    });
                                })
                                .catch(function() {
                                    document.getElementById('tableBody').innerHTML =
                                        '<tr><td colspan="4" style="color:red;text-align:center;">Failed to load data</td></tr>';
                                });
                        }
                        ...
                        function viewDept(deptno) {
                            fetch('/web08/api/detail?deptno=' + deptno)
                                .then(res => res.json())
                                .then(data => {
                        ...
                        function editDept(deptno) {
                            fetch('/web08/api/detail?deptno=' + deptno)
                                ...
                                .catch(function() { showToast('Failed to load data', 'error'); });
                        }

                        function deleteDept(deptno) {
                            if (confirm('Are you sure you want to delete department ' + deptno + '?')) {
                                fetch('/web08/api/delete?deptno=' + deptno, { method: 'POST' })
                                    .then(res => res.text())
                                    .then(function(msg) {
                        ...
                        function submitForm(e) {
                            ...
                            var url = mode === 'insert' ? '/web08/api/insert' : '/web08/api/update';
                            var body = 'deptno=' + deptno + '&dname=' + dname + '&loc=' + loc;
