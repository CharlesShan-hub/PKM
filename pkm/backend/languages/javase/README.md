# Javase

* Basic
    * [remember](notes/01-basic/remember.md)：备忘录
    * [Background](notes/01-basic/background.md)：Java基础、各个版本变化
    * [Hello World and Basic Java Structure](notes/01-basic/hello-world.md)
    * [Escape Character](notes/01-basic/escape-character.md)：转义字符
    * [Comment](notes/01-basic/comment.md)：注释
    * [package](notes/01-basic/package.md)：包
    * [Variable and Datatype](notes/01-basic/variable-and-datatype.md)：变量与数据类型
    * [operator](notes/01-basic/operator.md)：操作符
    * [Keyboard Input](notes/01-basic/keyboard-input.md)：键盘输入
    * [Branch Control](notes/01-basic/branch-control.md)：分支结构
    * [recursion](notes/01-basic/recursion.md)：递归
    *  [array](notes/01-basic/array.md)：数组
    * [Arrays](notes/01-basic/Arrays.md)：⚠️Comparator
* OOP Basic
    * [Introduction for Class and Object](notes/02-oop/class-and-object-introduce.md)：引入
    * [Attributes and Methods](notes/02-oop/attributes-and-methods.md)：属性和成员方法
    * [this](notes/02-oop/this.md)
    * [encapsulation](notes/02-oop/encapsulation.md)：封装，javabean
    * [static](notes/stage2/static.md)：类变量(静态变量) 与 类方法(静态方法)
    * [extends](notes/02-oop/extends.md)：继承，super
    * [Access Modifiers](notes/02-oop/access-modifiers.md)：public protected 缺省 private
    * [Object](notes/02-oop/Object.md)：所有类都继承了Object
    * [super](notes/02-oop/super.md)
    * [polymorphism](notes/02-oop/polymorphism.md)：多态
* OOP Advanced
    * [final](notes/03-oop-advanced/final.md)
    * [abstract](notes/03-oop-advanced/abstract.md)：抽象类与抽象方法
    * [interface](notes/03-oop-advanced/interface.md)：接口
    * [enum](notes/03-oop-advanced/enum.md)：枚举
    * [Code Block](notes/03-oop-advanced/code-block.md)：代码块
    * [Inner Class](notes/03-oop-advanced/inner-class.md)：内部类
    * [lambda](notes/03-oop-advanced/lambda.md)：表达式、方法引用
    * [exception](notes/03-oop-advanced/exception.md)：异常
    * [generic](notes/03-oop-advanced/generic.md)：泛型
    * [Collection](notes/03-oop-advanced/Collection.md)：集合
    * [stream](notes/03-oop-advanced/stream.md)：stream流
* Utils Classes
    * [String](notes/04-utils/String.md)：⚠️内存分布
    * [StringBuilder and StringBuffer](notes/04-utils/StringBuilder-and-StringBuffer.md)：⚠️扩容机制
    * [BigInteger and BigDecimal](notes/04-utils/BigIntegerBigDecimal.md)：大数运算
    * [WrapperClass](notes/04-utils/WrapperClass.md)：包装类
    * [Comparator](notes/04-utils/Comparator.md)：比较器
    * [Math](notes/04-utils/Math.md)：数学类
    * [Date](notes/04-utils/date.md)：日期类
    * [System](notes/04-utils/System.md)：系统类
    * [Random](notes/04-utils/Random.md)：随机数
* IO and Network
    * [File](notes/05-io/File.md)：文件
    * [Coding](notes/05-io/coding.md)：编码
    * [IO Stream](notes/05-io/io-stream.md)：IO流
    * [Commons IO](notes/05-io/common-io.md)：第三方IO工具
    * [network](notes/05-io/network.md)：网络
* Java Advanced
    * [Multiy Threading](notes/06-java-advanced/multiy-threading.md)：多线程
    * [reflect](notes/06-java-advanced/reflect.md)：反射
    * [annotation](notes/06-java-advanced/annotation.md)：注解
    * [Swing](notes/06-java-advanced/Swing.md)：GUI
    * [jvm](notes/06-java-advanced/jvm.md)
