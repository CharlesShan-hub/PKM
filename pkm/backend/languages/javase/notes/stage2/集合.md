...
![[../../oldassets/Collection-drawing|1000]]

`Collection` 和 `Iterable` 接口 API 案例

![[../../oldassets/Collection-interface-drawing|1000]]

`ArrayList`，`LinkedLisk`，`Vector` 对比

![[../../oldassets/list-drawing|1000]]

`HashSet` 与 `LinkedHashSet`

![[../../oldassets/hashset-drawing|1000]]
`Map`

![[../../oldassets/map-drawing| 1000]]
...
* 如果指定了类型（[[../03-oop-advanced/generic]]）那就只能传入特定的类型了
    ```java
    List<String> names = new ArrayList<>();
    ```
