...
- 数值类型
    ...
    - 整数类型
        ...
        - **INT**：-2147483648 ~ 2147483647，默认值为0，**四**个字节
        - **BIGINT**：-9223372036854775808 ~ 9223372036854775807，默认值为0，**八**个字节
    - 浮点数类型
        - FLOAT：单精度浮点数，**4**个字节（最多**5位**小数）
        - **DOUBLE**：双精度浮点数，**8**个字节（最多**16位**小数）
        - **DECIMAL**：定点数，用于精确计算，存储为字符串（语法：decimal(m, d)，例如：decimal(3, 2) 表示3个有效数字，2个小数。（有效数字最多65个，小数位最多30个））
    - 字符串类型
        - CHAR：固定长度字符串，最多255个字符。CHAR(4) 定义了一个固定长度的字符串列，包含的字符个数最大为 4。当插入的字符长度大于4，则报错（除非超过4个长度之后都是空格字符，则空格字符会自动被删除用来保证插入的成功）。
        - VARCHAR：可变长度字符串，最多16383个字符。实际占用的空间为字符串的实际长度加 1。
    ...
    - 二进制类型
        - BLOB：二进制对象，用于存储二进制数据，如图片、音频、视频等,（2^16 -1）
        - mediumblob：中等的，最大长度16777215个字节
        - LONGBLOB：更大的二进制大对象，用于存储更大的二进制数据，如图片、音频、视频等,（2^32 -1）（最大长度4GB的字节）
- 日期和时间类型
    ...
    - timestamp：4个字节，存储年月日+时分秒，格式：YYYY-MM-DD HH:MM:SS（从公元1980年~公元2040年）或者格式为 YYYYMMDDHHMMSS（采用这种格式不需要使用单引号，当然你使用单引号也可以）自动更新为当前时间
- 枚举类型
    - ENUM
- 集合类型
    - SET
...
* CHAR(4) 定义了一个固定长度的字符串列，包含的字符个数最大为 4。当插入的字符长度大于4，则**报错**（除非超过4个长度之后都是空格字符，则**空格字符会自动被删除**用来保证插入的成功）。
* 长度可变的字符串。varchar 的最大实际长度由最长的行的大小和使用的字符集确定，而**实际占用的空间为字符串的实际长度加 1**。例如，varchar(50) 定义了一个最大长度为 50 的字符串，如果插入的字符串只有 10 个字符，则实际存储的字符串为 10 个字符和一个字符串结束字符。varchar在值保存和检索时尾部的空格仍保留。
