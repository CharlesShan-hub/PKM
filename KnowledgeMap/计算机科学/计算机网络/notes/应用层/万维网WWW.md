...
<img src="resources/http.png" alt="http" style="zoom:50%;" />
...
<img src="resources/http2.png" alt="http2" style="zoom:67%;" />
...
<img src="resources/http4.png" alt="http数据报" style="zoom:67%;" />
...
* 使用鼠标单击一个万维网文档时，若该文档除有文本外，还有三幅gf图像，则在HTTP/1.0
...
* 仅需Wb服务器对HTTP报文进行响应，但不需要返回请求对象时，HTTP请求报文应
...
* 在如下条件下，计算使用非持续方式和持续方式请求一个Web页面所需的时间：
  ...
  2)一个Web页面中有l0幅gif图片，Web页面的基本HTML文件、HTTP请求报文
...
* 【2011统考真题】某主机的MAC地址为00-15-C5-C1-5E-28,IP地址为10.2.128.100（私有地址)。图1是网络拓扑，图2是该主机进行Web请求的一个以太网数据顿前80B的十六进制及ASCII码内容。
  ...
  <img src="resources/11-3.png" alt="11-3" style="zoom:50%;" />

  

  <img src="resources/11-4.png" alt="11-4" style="zoom:50%;" />
...
* 【2021统考真题】某网络拓扑如题47图所示，以太网交换机S通过路由器R与Internet互联。路由器部分接口、本地域名服务器、H1、H2的IP地址和MAC地址如图中所示。
  ...
  <img src="resources/21.png" alt="21" style="zoom:80%;" />
  ...
  3. | MAC地址           | 端口 |
     ...
     由步骤2)的分析可知，H2至少会接收到2个和此次Wb访问相关的帧。接收到的均是封装**ARP查询报文**的以太网帧；这些帧的目的MAC地址均是FF-FF-FF-FF-FF-FF。第一次是不知道DNS服务器，进行广播。第二次是不知道路由器，进行广播。