...
2. 邮件传送
   ...
   连接建立后，就可开始传送邮件。邮件的传送从MAL命令开始，MAL命令后面有发件人的地址。如`MAIL FROM:<hoopdog(@hust.edu.cn>`。若SMTP服务器已准备好接收邮件，则回答250OK。接着SMTP客户端发送一个或多个RCPT(收件人recipient的缩写)命令，格式为`RCPT TO:<收件人地址>`。每发送一个RCPT命令，都应有相应的信息从SMTP服务器返回，如250OK或550 No such user here(无此用户)。
