...
5. 一个线性表最常用的操作是存取任一指定序号的元素并在最后进行插入、删除操作，则利用（）存储方式可以节省时问。
   A. 顺序表
   B. 双链表
   C. 带头结点的双循环链表
   D. 单循环链表
...
6. 在n个元素的线性表的数组表示中，时间复杂度为 O(1) 的操作是（）。
   I. 访问第 i(1≤isn) 个结点和求第 i(2≤isn) 个结点的直接前驱
   II. 在最后一个结点后插入一个新的结点
   III. 删除第1个结点
   IV. 在第i（1sisn）个结点后插入一个结点
   A. I
   B. II、 III
   C. I、II
   D. I、II、III
...
8. 在一个长度为n的顾序表中删除第i（1sisn）个元素时，需向前移动（）个元素。
   A. n
   B. i-1
   C. nai
   D n-i+1
...
13. 【2011 统考真题】一个长度为L(L≥1）的升序序列S，处在第⌈L/2⌉个位置的数称为 S 的中位数。例如，若序列S=(11,13,15,17,19），则S的中位数是15，两个序列的中位数是含它们所有元素的升序序列的中位数。例如，若S2=（2,4,6,8,20)，则S1和S2的中位数是11。现在有两个等长升序序列A和B，试设计一个在时间和空间两方面都尽可能高效的算法，找出两个序列A和B的中位数。要求：
    1）给出算法的基本设计思想。
    2）根据设计思想，来用C或 C1+或 的白锤言描达年法号 关能之处给出注释
    3）说明你所设计算法的时问复杂度和空问复杂度
...
14. 【2013統考真题】已如一个整数序列A=(a0,a1,...,an-1)，其中0≤ai≤n (0≤i≤n)。若存在ap1=ap2=...=apm=x 且 m＞n/2 (0≤pk≤n，l≤k≤m），則称x为A的主元素。例如A=(0,5,5,3,5,7,5,5)，則5为主元素；又如A=(0,5,5,3,5,1,5,7)，则A中没有主元素。假设A中的n个元素保存在一个一维数组中，请设计一个尽可能高效的算法，找出A的主元素。若存在主元素，则输出该元素；否则输出-1。要求：
    1）给出算法的基本设计思想。
