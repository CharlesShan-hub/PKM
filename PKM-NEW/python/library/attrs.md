...
## 简介
* 官网： https://www.attrs.org/en/stable/index.html
* `attrs` 是一个Python库，它帮助开发者简化并加强类的创建。
	* 它允许你以声明式的方式定义类，自动为你处理属性的类型注解、默认值、初始化方法、比较方法、 repr 实现、属性验证等。
	* 
* `attrs`库在Python社区中非常受欢迎，因为它提高了代码的可读性和维护性，同时减少了冗余代码的编写。它被许多其他流行的库和框架所使用，如`SQLAlchemy`，`Tornado`和 `pytest`。
...
* **API模型**：在构建Web API时，`attrs`可以用于定义请求和响应模型。 
