...
* **`paint()`方法触发场景**  以下三种情况会调用`paint()`：  
	...
	3. **显式调用`repaint()`**  
	   • 程序主动请求刷新（如数据更新时）。  
* **思考题：如何验证这三种情况？**  **验证方法**（可代码实现或逻辑分析）：  
	1. **覆盖`paint()`方法**  
	   • 在自定义组件中重写`paint()`，添加日志输出或调试断点。  
	2. **模拟触发场景**  
	   • **最小化/恢复**：操作窗口并观察日志。  
	   • **调整窗口大小**：拖动边缘触发重绘。  
	   • **调用`repaint()`**：在按钮点击事件中手动调用。  
* **记忆技巧**：将`repaint()`理解为“主动刷新信号”，`paint()`是“实际执行绘画”。  
...
3. 事件：事件就是承载事件源状态改变时的对象，比如当键盘事件、鼠标事件、窗口事件  等等，会生成一个事件对象，该对象保存着当前事件很多信息，比如 KeyEvent 对象有 含有被按下键的Code值。java.awt.event 和 javax.swing.event 包中定义了各种事 件类型
4. 事件监听器接口：
	1. 当事件源产生一个事件，可以传送给事件监听者处理  
	2. 事件监听者实际上就是一个类，该类实现了某个事件监听器接口 比如前面我们案例中的MyPanle就是一个类，它实现了  KeyListener 接口，它就可以作为一个事件监听者，对接受到的事件进行处理  
	3. 事件监听器接口有多种，不同的事件监听器接口可以监听不同的事件，一个类可以实现多个监听接口这些接口在 java.awt.event 和 javax.swing.event 包中列出常用的事件监听器接口，查看 jdk 文档聚集了
