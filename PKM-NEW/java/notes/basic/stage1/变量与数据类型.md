...
* **变量类型**
	* 基本数据类型（8种）：
		...
		* 浮点型：float【4】(7 位小数)、double【8】（16 位小数）
		* 字符类型：char【2】
		* 布尔类型：boolean【1】
	* 引用数据类型：
		* 类（比如 String）
		* 接口
		* 数组
	* （ java char 2 字节，C char 1 字节，rust char 4 字节-Unicode）
* 加号的妙用
...
* 浮点变量
  ...
  * double 比 float 更加准确的案例
		```java
		double a = 2.123456789; // 2.123456789
		float b = 2.123456789F; // 2.1234567
		```
  * 陷阱（计算机表示不了0.3）
...
* 类型转换
  ...
  * 自动类型转换
    ...
    * 注意，默认转成最宽泛的
      ...
      ```java
      int a = 1;
      float n1 = a + 1.1;  // 错误，1.1 默认是 double
      float n2 = a + 1.1F; // 正确
      double n3 = a + 1.1; // 正确
      ```
